    ...
    I: Interner,
{
    ...
    pub(super) fn compute_projection_goal_new(
        &mut self,
        ...
        match goal.predicate.projection_term.kind(cx) {
            ty::AliasTermKind::ProjectionTy | ty::AliasTermKind::ProjectionConst => {
                ...
                let mut unconstrained_goal = goal;
                unconstrained_goal.predicate.term = unconstrained_term;
                ...
                let resp = self.probe(|_| ProbeKind::NormalizedSelfTyAssembly).enter(|ecx| {
                    ecx.var_values.var_values = extended_var_values;
                    ecx.current_goal_kind = super::eval_ctxt::CurrentGoalKind::NormalizesTo;
                ...
                let (nested_goals, certainty) = instantiate_and_apply_query_response(
                    self.delegate,
                    goal.param_env,
                    &extended_var_values.as_slice()[..resp.value.var_values.len()],
                    resp,
                    self.origin_span,
                );
            ...
            ty::AliasTermKind::InherentTy | ty::AliasTermKind::InherentConst => {
                self.normalize_inherent_associated_term_new(goal)
            }
            ty::AliasTermKind::OpaqueTy => self.normalize_opaque_type_new(goal),
            ty::AliasTermKind::FreeTy | ty::AliasTermKind::FreeConst => {
                self.normalize_free_alias_new(goal)
            }
            ty::AliasTermKind::UnevaluatedConst => self.normalize_anon_const_new(goal),
        }
    }
    ...
    pub fn instantiate_projection_term(
        ...
        let _ = self.eq(goal.param_env, goal.predicate.term, term);
    }
    ...
    I: Interner,
{
    ...
    fn consider_impl_candidate(
        ...
        ecx.probe_trait_candidate(CandidateSource::Impl(impl_def_id)).enter(|ecx| {
            ...
            let target_args = ecx.translate_args_new(
                goal,
                impl_def_id,
                impl_args,
                impl_trait_ref,
