...
enum CurrentGoalKind {
    ...
    /// productive steps.
    CoinductiveTrait,
    /// When probing `NormalizesTo` goal's candidates, we replace the expected term with an
    /// unconstrained, fresh inference var. That's because we should prevent the expectation
    /// from affecting the normalization. This would weaken inference however, as the nested
        ...
        match ty.kind() {
            ty::Alias(alias) if !ty.has_escaping_bound_vars() => {
                ...
                let normalizes_to = ty::PredicateKind::NormalizesTo(ty::NormalizesTo {
                    alias: ty::AliasTerm::new_from_args(self.cx(), alias.kind.def_id(), alias.args),
                    term: infer_ty.into(),
                });
                self.ecx.add_goal(
